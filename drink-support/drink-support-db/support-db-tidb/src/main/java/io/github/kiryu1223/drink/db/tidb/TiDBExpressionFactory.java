 ...
 * @since 3.0
 */
public class TiDBExpressionFactory extends DefaultSqlExpressionFactory {

    public TiDBExpressionFactory(IConfig config)
    {
        super(config);
    }
    ...
    public ISqlRecursionExpression recursion(ISqlQueryableExpression queryable, FieldMetaData parentId, FieldMetaData childId, int level) {
        return new TiDBRecursionExpression(queryable, parentId, childId, level);
    }
    ...
    {
        return new TiDBPivotExpression(tableExpression,aggregationColumn,aggregationType,transColumn,transColumnValues,tempRefExpression,pivotRefExpression);
    }
    ...
    {
        return new TiDBUnPivotExpression(tableExpression, newNameColumnName, newValueColumnName, newValueColumnType, transColumns, tempRefExpression, unPovitRefExpression);
    }
}
