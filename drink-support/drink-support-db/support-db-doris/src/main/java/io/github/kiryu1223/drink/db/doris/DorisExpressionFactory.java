 ...
 * @since 3.0
 */
public class DorisExpressionFactory extends DefaultSqlExpressionFactory {

    public DorisExpressionFactory(IConfig config)
    {
        super(config);
    }
    ...
    public ISqlRecursionExpression recursion(ISqlQueryableExpression queryable, FieldMetaData parentId, FieldMetaData childId, int level) {
        return new DorisRecursionExpression(queryable, parentId, childId, level);
    }
    ...
    {
        return new DorisPivotExpression(tableExpression,aggregationColumn,aggregationType,transColumn,transColumnValues,tempRefExpression,pivotRefExpression);
    }
    ...
    {
        return new DorisUnPivotExpression(tableExpression, newNameColumnName, newValueColumnName, newValueColumnType, transColumns, tempRefExpression, unPovitRefExpression);
    }
}
