 ...
 * @since 3.0
 */
public class GroupedQuery2<Key, T1, T2> extends QueryBase<GroupedQuery2<Key, T1, T2>, Key> {
     ...
     */
    public <R> EndQuery<R> select(@Expr(Expr.BodyType.Expr) Func1<Group2<Key, T1, T2>, R> expr) {
        throw new NotCompiledException();
    }

    public <R> EndQuery<R> select(ExprTree<Func1<Group2<Key, T1, T2>, R>> expr) {
        select(expr.getTree());
