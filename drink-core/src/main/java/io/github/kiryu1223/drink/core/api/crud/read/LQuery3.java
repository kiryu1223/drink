...
public class LQuery3<T1, T2, T3> extends QueryBase<LQuery3<T1, T2, T3>,T1> {
    ...
    public <R extends Result> LQuery<R> withTemp(@Expr(Expr.BodyType.Expr) Func3<T1, T2, T3, R> expr) {
        throw new NotCompiledException();
    }

    public <R extends Result> LQuery<R> withTemp(ExprTree<Func3<T1, T2, T3, R>> expr) {
        select(expr.getTree());
        withTempQuery();
